  ...
  async (session: Session, { fulfillWithValue, rejectWithValue }) => {
    try {
      if (session.provider_token) {
        const calendarsResponse = await calendarsService.fetchCalendars(
          session.provider_token,
        );
        if (!calendarsResponse) throw new Error("Failed to fetch calendars");

        const fetchEventsPromises = calendarsResponse.items.map(
          async (calendar: ICalendar) => {
            const eventsResponse = await calendarsService.fetchCalendarEvents(
              session.provider_token!,
              calendar.id,
            );
...
const calendarsSlice = createSlice({
  ...
  reducers: {
    ...
    toggleCalendarsVisibility: (state, action) => {
      state.calendars = state.calendars.map((calendar) => ({
        ...calendar,
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(fetchCalendarsWithEvents.fulfilled, (state, action) => {
        if (action.payload) {
          state.calendars = action.payload.calendars.map(
            (calendar: ICalendar) => {
              const calendarEvents = action.payload!.events.find(
                (eventItem: ICalendarEvent) =>
                  eventItem.calendarId === calendar.id,
              );
              return {
                ...calendar,
...
export const { setItems, toggleCalendarVisibility, toggleCalendarsVisibility } =
